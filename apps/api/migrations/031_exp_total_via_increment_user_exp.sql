-- Ensure total exp (user_levels) is always updated from Node via increment_user_exp.
-- Split: increment_user_exp only updates total; increment_daily_exp_with_milestones only updates daily + milestones.
...
GRANT EXECUTE ON FUNCTION "public"."grant_user_exp_total"("p_user_id" "uuid", "p_exp_seconds" bigint, "p_date" date, "p_reason" text) TO "service_role";
GRANT EXECUTE ON FUNCTION "public"."grant_user_exp_daily_only"("p_user_id" "uuid", "p_date" date, "p_exp_seconds" bigint) TO "service_role";
