...
const (
	...
	processingPref = "webhook:clerk:processing:"
	processedPref  = "webhook:clerk:processed:"
)
...
func ProcessDelivery(ctx context.Context, deliveryID string, evt map[string]interface{}) error {
	if processed, _ := isProcessed(ctx, deliveryID); processed {
		return nil
	}
	if !acquireLock(ctx, deliveryID) {
		return nil
	}
	if err := handleEvent(ctx, evt); err != nil {
		releaseLock(ctx, deliveryID)
		return err
	}
...
func isProcessed(ctx context.Context, id string) (bool, error) {
	c := redisx.Client()
	if c == nil {
		return false, nil
	}
	v, err := c.Get(ctx, processedPref+id).Result()
	if err != nil {
		return false, nil
	}
	return v == "1", nil
...
func acquireLock(ctx context.Context, id string) bool {
	c := redisx.Client()
	if c == nil {
		return true
	}
	ok, err := c.SetNX(ctx, processingPref+id, "1", processingTTL).Result()
	if err != nil {
		return false
	}
	return ok
...
func releaseLock(ctx context.Context, id string) {
	c := redisx.Client()
	if c == nil {
		return
	}
	_ = c.Del(ctx, processingPref+id).Err()
}
...
func markProcessed(ctx context.Context, id string) {
	...
	_ = c.Set(ctx, processedPref+id, "1", processedTTL).Err()
	_ = c.Del(ctx, processingPref+id).Err()
}
