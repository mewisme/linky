...
type FavoriteRow struct {
	ID            string `json:"id"`
	UserID        string `json:"user_id"`
	FavoritedID   string `json:"favorited_user_id"`
	CreatedAt     string `json:"created_at"`
}
...
func GetFavoritesByUserID(ctx context.Context, userID string) ([]string, error) {
	...
	raw, _, err := c.From("favorites").
		Select("favorited_user_id", "exact", false).
		Eq("user_id", userID).
		ExecuteWithContext(ctx)
	if err != nil {
		return nil, err
	...
	for _, r := range rows {
		out = append(out, r.FavoritedID)
	}
	return out, nil
}

func ListFavorites(ctx context.Context, userID string) ([]map[string]any, error) {
	c := Client()
	if c == nil {
		return nil, errors.New("supabase: not configured")
	}
	raw, _, err := c.From("favorites").
		Select("id, favorited_user_id, created_at, users!favorites_favorited_fkey(id, first_name, last_name, avatar_url, country)", "exact", false).
		Eq("user_id", userID).
...
func CreateFavorite(ctx context.Context, userID, favoritedID string) (map[string]any, error) {
	c := Client()
	if c == nil {
		return nil, errors.New("supabase: not configured")
	}
	body := map[string]any{
		"user_id":           userID,
		"favorited_user_id": favoritedID,
	}
	raw, _, err := c.From("favorites").
		Upsert(body, "user_id,favorited_user_id", "representation", "exact").
		ExecuteWithContext(ctx)
	if err != nil {
		return nil, err
	}
...
func DeleteFavorite(ctx context.Context, userID, favoritedID string) error {
	c := Client()
	if c == nil {
		return errors.New("supabase: not configured")
	}
	_, _, err := c.From("favorites").
		Delete("", "exact").
		Eq("user_id", userID).
		Eq("favorited_user_id", favoritedID).
		ExecuteWithContext(ctx)
	return err
}
