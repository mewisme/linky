...
func handleCreateReport(c echo.Context) error {
	...
	if row != nil {
		go contexts.OnReportCreated(context.Background(), row.ID)
	}
...
func registerFavoritesRoutes(g *echo.Group) {
	...
	g.DELETE("/:favorited_user_id", handleDeleteFavorite)
}
...
func handleListFavorites(c echo.Context) error {
	...
	rows, err := supax.ListFavorites(c.Request().Context(), uid)
	if err != nil {
		return httpx.SendError(c, 500, "Internal Server Error",
...
func handleCreateFavorite(c echo.Context) error {
	...
	rawBody, _ := io.ReadAll(c.Request().Body)
	var input struct {
		FavoritedUserID string `json:"favorited_user_id"`
	}
	_ = json.Unmarshal(rawBody, &input)
	if input.FavoritedUserID == "" {
		return httpx.SendError(c, 400, "Bad Request",
			httpx.UM("FAVORITE_TARGET_REQUIRED", "favoriteTargetRequired", "favorited_user_id is required"))
	}
	if input.FavoritedUserID == uid {
		return httpx.SendError(c, 400, "Bad Request",
			httpx.UMDetail("FAVORITE_SELF", "Cannot favorite yourself"))
	}
	row, err := supax.CreateFavorite(c.Request().Context(), uid, input.FavoritedUserID)
	if err != nil {
		return httpx.SendError(c, 500, "Internal Server Error",
			httpx.UM("FAILED_CREATE_FAVORITE", "failedCreateFavorite", "Failed to create favorite"))
	}
	return c.JSON(http.StatusCreated, row)
}
...
func handleDeleteFavorite(c echo.Context) error {
	...
	target := c.Param("favorited_user_id")
	if target == "" {
		return httpx.SendError(c, 400, "Bad Request",
			httpx.UM("FAVORITE_TARGET_REQUIRED", "favoriteTargetRequired", "favorited_user_id is required"))
	}
	if err := supax.DeleteFavorite(c.Request().Context(), uid, target); err != nil {
		return httpx.SendError(c, 500, "Internal Server Error",
			httpx.UM("FAILED_DELETE_FAVORITE", "failedDeleteFavorite", "Failed to delete favorite"))
	}
