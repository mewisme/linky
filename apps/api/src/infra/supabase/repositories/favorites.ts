...
export async function getFavoritesByUserId(userId: string): Promise<string[]> {
  ...
  if (error) {
    logger.error(toLoggableError(error), "Error fetching favorites");
    throw error;
  }
...
export async function checkFavoriteExists(userId: string, favoriteUserId: string): Promise<boolean> {
  ...
  if (error) {
    logger.error(toLoggableError(error), "Error checking favorite exists");
    throw error;
  }
...
export async function createFavorite(userId: string, favoriteUserId: string): Promise<FavoriteRecord> {
  ...
  if (error) {
    logger.error(toLoggableError(error), "Error creating favorite");
    throw error;
  }
...
export async function deleteFavorite(userId: string, favoriteUserId: string): Promise<boolean> {
  ...
  if (error) {
    logger.error(toLoggableError(error), "Error deleting favorite");
    throw error;
  }
...
export async function getFavoriteLimitForToday(userId: string): Promise<FavoriteLimitRecord | null> {
  ...
  if (error) {
    logger.error(toLoggableError(error), "Error fetching favorite limit");
    throw error;
  }
...
export async function incrementFavoriteLimit(userId: string, dailyLimit: number = 10): Promise<FavoriteLimitRecord> {
  ...
  if (error) {
    logger.error(toLoggableError(error), "Error incrementing favorite limit");
    throw error;
  }
  ...
  if (data.used_count > 1) {
    ...
    if (updateError) {
      logger.error(toLoggableError(updateError), "Error updating favorite limit");
      throw updateError;
    }
...
export async function decrementFavoriteLimit(userId: string): Promise<boolean> {
  ...
  if (error) {
    logger.error(toLoggableError(error), "Error decrementing favorite limit");
    throw error;
  }
...
export async function getFavoriteCreationDate(userId: string, favoriteUserId: string): Promise<string | null> {
  ...
  if (error) {
    logger.error(toLoggableError(error), "Error fetching favorite creation date");
    throw error;
  }
...
export async function getFavoritesWithStats(userId: string): Promise<FavoriteWithStats[]> {
  ...
  if (error) {
    logger.error(toLoggableError(error), "Error fetching favorites with stats");
    throw error;
  }
