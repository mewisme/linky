...
func ApplyRealtimeCallProjection(progress *Insights, unpersistedElapsedSeconds, projectedExpGain int, favoriteRelation string, cfg exp.Config) *Insights {
	if progress == nil {
		return nil
	}

	streakForBonus := streakCountForExpBonus(progress, unpersistedElapsedSeconds)
	projectedExpGain = exp.EffectiveSeconds(projectedExpGain, streakForBonus, progress.CurrentLevel, favoriteRelation, cfg)
	...
	out.ExpBonuses = exp.ActiveBonuses(streakCountForExpBonus(&out, unpersistedElapsedSeconds), out.CurrentLevel, favoriteRelation, cfg)
	return &out
}
