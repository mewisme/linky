...
func handleUserMe(c echo.Context) error {
	...
	row, err := user.GetMe(c.Request().Context(), clerkID, cfCountry)
	if err != nil {
...
func handleUpdateMeCountry(c echo.Context) error {
	...
	updated, err := user.UpdateCountry(c.Request().Context(), input.ClerkUserID, input.Country)
	if err != nil {
		return httpx.SendError(c, 500, "Internal Server Error",
...
func handleUserLevelMe(c echo.Context) error {
	...
	data, err := user.GetUserLevelData(c.Request().Context(), uid)
	if err != nil {
		return httpx.SendError(c, 500, "Internal Server Error",
...
func handleUserStreakMe(c echo.Context) error {
	...
	data, err := user.GetUserStreakData(c.Request().Context(), uid)
	if err != nil {
		return httpx.SendError(c, 500, "Internal Server Error",
...
func handleUserProgressMe(c echo.Context) error {
	...
	tz := user.ResolveTimezone(c.Request().Context(), uid, nil)
	insights, err := user.GetProgressInsights(c.Request().Context(), uid, tz)
	if err != nil {
...
func handleBlocksMe(c echo.Context) error {
	...
	rows, err := user.ListBlocks(c.Request().Context(), uid)
	if err != nil {
		return httpx.SendError(c, 500, "Internal Server Error",
...
func handleCreateBlock(c echo.Context) error {
	...
	row, err := user.CreateBlock(c.Request().Context(), uid, input.BlockedUserID)
	if err != nil {
		if errors.Is(err, user.ErrBlockSelf) || errors.Is(err, user.ErrAlreadyBlocked) {
			return httpx.SendError(c, 400, "Bad Request", httpx.UMDetail("BLOCK_VALIDATION", err.Error()))
		}
		return httpx.SendError(c, 500, "Internal Server Error",
...
func handleDeleteBlock(c echo.Context) error {
	...
	if err := user.DeleteBlock(c.Request().Context(), uid, blocked); err != nil {
...
func handleListCallHistory(c echo.Context) error {
	...
	rows, count, err := videochat.ListCallHistory(c.Request().Context(), uid, limit, offset)
	if err != nil {
		return httpx.SendError(c, 500, "Internal Server Error",
...
func handleGetCallHistoryItem(c echo.Context) error {
	...
	row, err := videochat.GetCallHistoryItem(c.Request().Context(), uid, id)
	if err != nil {
...
func handleListNotifications(c echo.Context) error {
	...
	rows, err := user.ListNotifications(c.Request().Context(), uid, limit, offset, unreadOnly)
	if err != nil {
		return httpx.SendError(c, 500, "Internal Server Error",
...
func handleUnreadCount(c echo.Context) error {
	...
	count, err := user.UnreadNotificationCount(c.Request().Context(), uid)
	if err != nil {
		return httpx.SendError(c, 500, "Internal Server Error",
...
func handleMarkRead(c echo.Context) error {
	...
	if err := user.MarkNotificationRead(c.Request().Context(), id, uid); err != nil {
		return httpx.SendError(c, 500, "Internal Server Error",
...
func handleMarkAllRead(c echo.Context) error {
	...
	if err := user.MarkAllNotificationsRead(c.Request().Context(), uid); err != nil {
		return httpx.SendError(c, 500, "Internal Server Error",
