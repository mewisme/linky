...
} from "../../lib/supabase/queries/changelogs.js";
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    const { data: changelogs, count } = await getAllChangelogsForAdmin({
      limit,
      offset,
      orderBy,
    });
...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    const changelog = await createChangelog(params);
...
router.put("/:id", async (req: Request, res: Response) => {
  try {
    ...
    const changelog = await updateChangelog(id, params);
