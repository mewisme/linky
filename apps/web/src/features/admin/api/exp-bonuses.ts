...
export async function getAdminExpBonuses(
  ...
  return withSentryQuery("getAdminExpBonuses", async () =>
    serverFetch<AdminAPI.ExpBonuses.Get.Response>(
      backendUrl.admin.expBonuses(searchParams),
    ),
  );
}
...
export async function deleteExpBonus(
  id: string,
): Promise<AdminAPI.ExpBonuses.Delete.Response> {
