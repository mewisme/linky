...
export async function getAdminInterestTags(
  ...
  return withSentryQuery("getAdminInterestTags", async () =>
    serverFetch<AdminAPI.InterestTags.Get.Response>(
      backendUrl.admin.interestTags(searchParams),
    ),
  );
}
...
export async function deleteInterestTag(
  id: string,
): Promise<AdminAPI.InterestTags.Delete.Response> {
...
export async function hardDeleteInterestTag(
  id: string,
): Promise<AdminAPI.InterestTags.HardDelete.Response> {
