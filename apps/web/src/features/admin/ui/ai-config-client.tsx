...
function mergeFormFromResponse(
  res: AdminAPI.AI.Config.Response,
): AdminAPI.AI.Settings {
  ...
  return {
    ...
    models: {
      chat: {
        ...
        report_summary:
          admin.models?.chat?.report_summary ?? chatEff.report_summary ?? "",
      },
    ...
    timeouts: {
      request_ms:
        admin.timeouts?.request_ms ?? eff.timeouts?.request_ms ?? 60000,
      embedding_ms:
        admin.timeouts?.embedding_ms ?? eff.timeouts?.embedding_ms ?? 60000,
    },
    embedding: {
      user_api_batch_size:
        admin.embedding?.user_api_batch_size ??
        (eff.embedding as AdminAPI.AI.EmbeddingJobConfig | undefined)
          ?.user_api_batch_size ??
        8,
      dimension:
...
export function AdminAIConfigClient() {
  ...
  const { data, isFetching, refetch } = useQuery({
    ...
    queryFn: () =>
      fetchFromActionRoute<AdminAPI.AI.Config.Response>("/api/admin/ai/config"),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (settings: AdminAPI.AI.Settings) =>
      fetchFromActionRoute<AdminAPI.AI.Config.PutResponse>(
        "/api/admin/ai/config",
        {
          method: "PUT",
    ...
    onError: (error) => {
      toast.error(
        resolveActionErrorMessage(error, tRoot, "admin.aiConfig.saveFailed"),
      );
    },
  });
          ...
          <Card>
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((p) => ({
                      ...
                      timeouts: {
                        ...p.timeouts,
                        request_ms: Number(e.target.value) || undefined,
                      },
                    }))
                  }
                />
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm((p) => ({
                      ...
                      timeouts: {
                        ...p.timeouts,
                        embedding_ms: Number(e.target.value) || undefined,
                      },
                    }))
                  }
                />
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {t("embeddingDimensionHint", {
                    N: form.embedding?.dimension ?? 3072,
                  })}
                </p>
              </div>
            </CardContent>
