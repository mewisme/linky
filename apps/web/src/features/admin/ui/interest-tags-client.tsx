...
const EmojiPickerLazy = dynamic(
    ...
    import("@ws/ui/components/ui/emoji-picker").then((mod) => ({
      default: ({
        ...
        <mod.EmojiPicker
          className={className}
          onEmojiSelect={
            onEmojiSelect ? (e) => onEmojiSelect(e.emoji) : undefined
          }
        >
  ...
  {
    loading: () => (
      <div className="h-[342px] flex items-center justify-center">
...
const InterestTagsDataTable = dynamic(() =>
  import("@/shared/ui/data-table/interest-tags/data-table").then((mod) => ({
    default: mod.InterestTagsDataTable,
  })),
);

const ImportInterestTagsDialog = dynamic(
  () =>
    import("@/shared/ui/data-table/interest-tags/import-interest-tags-dialog").then(
      (mod) => ({ default: mod.ImportInterestTagsDialog }),
    ),
  { ssr: false },
);
...
export function InterestTagsClient({ initialData }: InterestTagsClientProps) {
  ...
  const [editingTag, setEditingTag] =
    useState<AdminAPI.InterestTags.InterestTag | null>(null);
  const [formData, setFormData] = useState<
    Partial<AdminAPI.InterestTags.Create.Body>
  >({
    name: "",
    description: "",
    category: "",
  ...
  const { data, isPending, isFetching, refetch } = useQuery({
    ...
    queryFn: () =>
      fetchFromActionRoute<AdminAPI.InterestTags.Get.Response>(
        "/api/admin/interest-tags",
      ),
    initialData,
    staleTime: Infinity,
  });
  ...
  const upsertMutation = useMutation({
    mutationFn: async (
      payload: Partial<AdminAPI.InterestTags.Create.Body> & { id?: string },
    ) => {
      ...
      if (isUpdate) {
        return fetchFromActionRoute<AdminAPI.InterestTags.Update.Response>(
          `/api/admin/interest-tags/${encodeURIComponent(tagId)}`,
          {
            ...
            body: JSON.stringify(
              requestPayload as AdminAPI.InterestTags.Update.Body,
            ),
          },
        );
      }
      return fetchFromActionRoute<AdminAPI.InterestTags.Create.Response>(
        "/api/admin/interest-tags",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(
            requestPayload as AdminAPI.InterestTags.Create.Body,
          ),
        },
      );
    },
  ...
  const rowCallbacks = {
    ...
    onActivate: (tag: AdminAPI.InterestTags.InterestTag) =>
      upsertMutation.mutate({ ...tag, is_active: true }),
    onDelete: (tag: AdminAPI.InterestTags.InterestTag) =>
      deleteMutation.mutate({ id: tag.id, hard: false }),
    onDeletePermanently: (tag: AdminAPI.InterestTags.InterestTag) =>
      deleteMutation.mutate({ id: tag.id, hard: true }),
  };
          ...
          <div className="flex items-center gap-2">
            <SimpleTooltip content={tif("importJson")}>
              <Button
                ...
                data-testid="admin-interest-tags-import-button"
              >
            ...
            <SimpleTooltip content={tif("addNewTag")}>
              <Button
                ...
                size="sm"
                data-testid="admin-interest-tag-create-button"
              >
        ...
        <DialogContent className="sm:max-w-[500px]">
          <form onSubmit={onFormSubmit}>
            <DialogHeader>
              ...
              <DialogDescription>{tif("modalDescription")}</DialogDescription>
            </DialogHeader>
            <div className="grid gap-6 py-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    id="name"
                    placeholder={tif("displayNamePlaceholder")}
                    value={formData.name}
                    required
                    onChange={(e) =>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="category"
                    placeholder={tif("categoryPlaceholder")}
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2 flex-row">
                  <Input
                    id="icon"
                    placeholder={tif("iconPlaceholder")}
                    value={formData.icon || ""}
                    onChange={(e) =>
