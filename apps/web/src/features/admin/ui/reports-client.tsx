...
const AdminReportsDataTable = dynamic(() =>
  import("@/shared/ui/data-table/admin-reports/data-table").then((mod) => ({
    default: mod.AdminReportsDataTable,
  })),
);
...
export function ReportsClient({ initialData }: ReportsClientProps) {
  ...
  const buildQueryParams = (): Record<string, string> => {
    ...
    if (reporterUserIdFilter.trim())
      params.reporter_user_id = reporterUserIdFilter.trim();
    if (reportedUserIdFilter.trim())
      params.reported_user_id = reportedUserIdFilter.trim();
    return params;
  };
  ...
  const {
    ...
    queryKey: [
      "admin-reports",
      statusFilter,
      reporterUserIdFilter,
      reportedUserIdFilter,
    ],
    queryFn: () => {
      const qs = new URLSearchParams(buildQueryParams()).toString();
      return fetchFromActionRoute<AdminAPI.Reports.Get.Response>(
        ...
        <div className="flex flex-col md:flex-row gap-4 p-4 border rounded-lg bg-card">
          <div className="flex-1 space-y-2">
            ...
            <Select
              value={statusFilter}
              onValueChange={(value) =>
                setStatusFilter(value as AdminAPI.Reports.ReportStatus)
              }
            >
