...
export function CompareEmbeddingsModal({
  ...
  const handleSubmit = async () => {
    ...
    setResult(null);
    try {
      const data = await fetchFromActionRoute<EmbeddingCompareResponse>(
        "/api/admin/embeddings/compare",
        {
          method: "POST",
      ...
      setError(
        resolveActionErrorMessage(err, tRoot, "admin.embedding.compareFailed"),
      );
    } finally {
      setIsLoading(false);
    }
      ...
      {result && (
        <div className="space-y-2 rounded-md border p-3">
          <p className="text-sm font-medium">
            {te("similarityScore", {
              score: (result.similarity * 100).toFixed(2),
            })}
          </p>
        </div>
      )}
      ...
      <Drawer open={open} onOpenChange={handleOpenChange} dismissible={false}>
        <DrawerContent className="max-h-[90vh] overflow-y-auto sm:max-w-md">
          <DrawerHeader>
            ...
            <DrawerDescription>{te("compareDescription")}</DrawerDescription>
          </DrawerHeader>
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>{te("compareDescription")}</DialogDescription>
        </DialogHeader>
        {body}
      </DialogContent>
    </Dialog>
