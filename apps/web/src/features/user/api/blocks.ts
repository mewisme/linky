...
export async function getBlockedUsers(): Promise<BlockedUsersResponse> {
  return withSentryQuery("getBlockedUsers", async () =>
    serverFetch<BlockedUsersResponse>(backendUrl.users.blocksMe()),
  );
}

export async function blockUser(userId: string): Promise<BlockRecord> {
  return withSentryAction("blockUser", async () =>
    serverFetch<BlockRecord>(backendUrl.users.blocks(), {
      method: "POST",
...
export async function unblockUser(userId: string): Promise<void> {
  return withSentryAction("unblockUser", async () => {
    await serverFetch<void>(backendUrl.users.blockByUserId(userId), {
      method: "DELETE",
    });
  });
}
