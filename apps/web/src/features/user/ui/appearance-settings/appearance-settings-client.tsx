...
export function AppearanceSettingsClient({
  ...
  const initialSidebar = normalizeUserSidebarPreferences(
    initialSettings.sidebar,
  );
  const initialCall = normalizeUserCallPreferences(initialSettings.call);
  const initialLocale =
    normalizeUserLanguage(initialSettings.language) ??
    useLocalePreferenceStore.getState().locale;
  const [defaultMuteMic, setDefaultMuteMic] = useState(
    initialCall.default_mute_mic,
  );
  const [defaultDisableCamera, setDefaultDisableCamera] = useState(
    initialCall.default_disable_camera,
  );
  const callQualityRef = useRef(initialCall.quality);
  const [shaderType, setShaderType] = useState<ShaderType>(initialShader.type);
  const [shaderPreset, setShaderPreset] = useState<ShaderPresetType>(
    initialShader.preset,
  );
  const [shaderAnimationEnabled, setShaderAnimationEnabled] = useState<boolean>(
    !initialShader.disabled,
  );
  const [shaderProps, setShaderProps] = useState<
    ShaderRenderMap[ShaderType] | undefined
  >(initialShader.props);
  const [savedShaderProps, setSavedShaderProps] = useState<
    ShaderRenderMap[ShaderType] | undefined
  >(initialShader.props);
  const [isSavingShaderDetails, setIsSavingShaderDetails] = useState(false);
  const [activeTab, setActiveTab] = useState<
    "language" | "shader" | "video" | "sidebar"
  >("language");
  const [sidebarVariant, setSidebarVariant] = useState<SidebarVariant>(
    initialSidebar.variant,
  );
  const [sidebarCollapsible, setSidebarCollapsible] =
    useState<SidebarCollapsible>(initialSidebar.collapsible);
  ...
  useEffect(() => {
    const applyStoredLocale = () => {
      ...
      const resolved =
        normalizeUserLanguage(initialSettings.language) ?? persisted;
      setUiLocaleDraft(resolved);
    };
    if (useLocalePreferenceStore.persist.hasHydrated()) {
      applyStoredLocale();
    }
    return useLocalePreferenceStore.persist.onFinishHydration(
      applyStoredLocale,
    );
  ...
  const hasUnsavedShaderDetails = useMemo(() => {
    return (
      JSON.stringify(shaderProps ?? {}) !==
      JSON.stringify(savedShaderProps ?? {})
    );
  ...
  const debouncedAutosave = useDebounceCallback(
    (...args: unknown[]) => {
      ...
      if (
        payloadKey === lastSavedKeyRef.current ||
        payloadKey === inFlightPayloadKeyRef.current
      ) {
        return;
      }
      inFlightPayloadKeyRef.current = payloadKey;
      void fetchFromActionRoute<UsersAPI.UserSettings.PatchMe.Response>(
        "/api/users/settings",
        {
          method: "PATCH",
        ...
        .catch((error: unknown) => {
          toast.error(
            resolveActionErrorMessage(
              error,
              tRoot,
              "settings.appearancePage.updateFailed",
            ),
          );
        })
        .finally(() => {
  ...
  const saveShaderDetails = async () => {
    ...
    try {
      await fetchFromActionRoute<UsersAPI.UserSettings.PatchMe.Response>(
        "/api/users/settings",
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    ...
    >
      ...
      >
        <TabsList
          variant={"line"}
          className="w-full justify-start overflow-x-auto whitespace-nowrap overflow-y-hidden"
        >
          <TabsTrigger value="language" className="shrink-0">
            {t("appearancePage.language")}
          </TabsTrigger>
          <TabsTrigger value="shader" className="shrink-0">
            {t("appearancePage.shaderSection")}
          </TabsTrigger>
          <TabsTrigger value="video" className="shrink-0">
            {t("appearancePage.videoDefaults")}
          </TabsTrigger>
          <TabsTrigger value="sidebar" className="shrink-0">
