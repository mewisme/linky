...
const BlockedUsersDataTable = dynamic(
  () =>
    import("@/shared/ui/data-table/blocked-users/data-table").then((mod) => ({
      default: mod.BlockedUsersDataTable,
    })),
  { ssr: false },
);
...
export function BlockedUsersClient({ initialData }: Props) {
  ...
  const handleRefresh = () => {
    startFetching(async () => {
      try {
        const res = await fetchFromActionRoute<BlockedUsersResponse>(
          "/api/users/blocks/me",
        );
