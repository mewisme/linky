...
export function PasswordModal({
  ...
  const changeWithReverify = useReverification((newPwd: string) =>
    user.updatePassword({ newPassword: newPwd }),
  );

  const setWithReverify = useReverification((newPwd: string) =>
  ...
  const formBody = (
    <>
      ...
      <div className="space-y-2">
        <Label>
          {isChange ? t("labelConfirmNewPassword") : t("labelConfirmPassword")}
        </Label>
        <div className="relative">
          <Input
