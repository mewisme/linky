...
function providerDisplayName(
  ...
  ts: (
    key: "providerGoogle" | "providerFacebook" | "providerDiscord",
  ) => string,
): string {
...
export function ProviderList({ userProviders }: ProviderListProps) {
  ...
  const mergedProviders = providers
    .map((provider) => {
      const external = userProviders.find((acc) => acc.provider === provider);
  ...
  return (
    <>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{ts("disconnectTitle")}</AlertDialogTitle>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>{ts("keepConnected")}</AlertDialogCancel>

            <AlertDialogAction onClick={confirmDestroy}>
              {ts("yesDisconnect")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
