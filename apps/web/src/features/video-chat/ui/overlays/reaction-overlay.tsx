...
function toNormalizedReactionTypes(type: string | string[]): string[] {
  ...
  const normalizedTypes = types.filter(
...
function buildDisplayTypesForInstances(
  types: string[],
  ...
  const selected = shuffle(resolvedTypes).slice(
    0,
    Math.min(instanceCount, resolvedTypes.length),
  );
...
export function ReactionOverlay() {
  ...
  const [reactionInstancesById, setReactionInstancesById] = useState<
    Record<string, ReactionInstance[]>
  >({});
  const [completedCountByReaction, setCompletedCountByReaction] = useState<
    ...
    setReactionInstancesById((prev) => {
      ...
      for (const reaction of reactions) {
        ...
        if (reaction.isLocal && reaction.tapPosition) {
          ...
          const reactionIndex = reactionIndexStr
            ? parseInt(reactionIndexStr, 10)
            : 0;
          ...
          for (let i = 0; i < burstCount; i++) {
            ...
            instances.push({
              ...
              scale: randomScale,
              delay:
                (isNaN(reactionIndex) ? 0 : reactionIndex * 0.1) +
                i * randomInRange(0.04, 0.1),
              duration: randomInRange(
                ANIMATION_DURATION_MIN,
                ANIMATION_DURATION_MAX,
              ),
  ...
  useEffect(() => {
    ...
    completedCountByReaction,
    reactionInstancesById,
    reactions,
    removeReaction,
  ]);
              ...
              <motion.div
                ...
                onAnimationComplete={() =>
                  handleReactionComplete(
                    reactionId,
                    instanceCountByReaction[reactionId] ?? 1,
                  )
                }
