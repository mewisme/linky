...
import { fetchData, patchData } from "@/lib/http/client-api";
...
export async function getNotifications(
  ...
  return fetchData<NotificationsResponse>(
    apiUrl.notifications.me(searchParams),
    { token },
  );
}
...
export async function getUnreadCount(
  ...
  return fetchData<UnreadCountResponse>(apiUrl.notifications.unreadCount(), {
    token,
  });
}
