...
export async function deleteObject(
  ...
  return deleteData<S3API.DeleteObject.Response>(
    apiUrl.media.s3ObjectByKey(key),
    { token },
  );
}
...
export async function startMultipartUpload(
  ...
  return postData<S3API.StartMultipart.Response>(
    apiUrl.media.s3MultipartStart(),
    {
      token,
      body,
    },
...
export async function getPartUploadUrl(
  ...
  return fetchData<S3API.GetPartUploadUrl.Response>(
    withQuery(apiUrl.media.s3MultipartPart(uploadId, partNumber), {
      ...params,
    }),
    { token },
  );
...
export async function completeMultipartUpload(
  ...
  return postData<S3API.CompleteMultipart.Response>(
    apiUrl.media.s3MultipartComplete(),
    {
      token,
      body,
    },
...
export async function abortMultipartUpload(
  ...
  return postData<S3API.AbortMultipart.Response>(
    apiUrl.media.s3MultipartAbort(),
    {
      token,
      body,
    },
