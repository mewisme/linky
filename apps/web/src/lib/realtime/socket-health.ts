...
class SocketHealthMonitor {
  ...
  private roomPingHandler:
    | ((data: { timestamp?: number; roomId?: string }) => void)
    | null = null;
  ...
  private checkHealth(): void {
    ...
    if (
      isInCall &&
      timeSinceLastEvent > SOCKET_SILENCE_THRESHOLD_MS &&
      this.lastEventTimestamp > 0
    ) {
      if (!this.halfDeadDetected) {
        Sentry.logger.warn(
          "[SocketHealth] Half-dead socket detected - connected but silent",
          { timeSinceLastEvent },
        );
        ...
        if (now - this.lastResyncAttempt > RESYNC_DEBOUNCE_MS) {
          ...
          Sentry.logger.info(
            "[SocketHealth] Triggering resync on half-dead detection",
          );
      ...
      if (!this.resyncTimeout) {
        this.resyncTimeout = setTimeout(() => {
          Sentry.logger.error(
            "[SocketHealth] Socket resync timeout - forcing teardown",
          );
    ...
    if (this.reconnectDetected && isInCall) {
      Sentry.logger.info(
        "[SocketHealth] Reconnect detected during active call - resync required",
      );
      this.context.onResyncRequired();
      this.reconnectDetected = false;
    }
  ...
  start(context: SocketHealthContext): void {
    ...
    this.visibilityChangeHandler = () => {
      ...
      if (wasBackgrounded && !this.isBackgrounded) {
        Sentry.logger.info(
          "[SocketHealth] App returned to foreground - resuming health checks",
        );
        this.trackEvent();
      } else if (!wasBackgrounded && this.isBackgrounded) {
        Sentry.logger.info(
          "[SocketHealth] App backgrounded - pausing health checks to prevent false positives",
        );
        if (this.resyncTimeout) {
          clearTimeout(this.resyncTimeout);
          this.resyncTimeout = null;
    ...
    this.connectHandler = () => {
      const wasConnected =
        this.lastEventTimestamp > 0 &&
        Date.now() - this.lastEventTimestamp < 5000;
      if (wasConnected) {
    ...
    this.roomPingHandler = (data: { timestamp?: number; roomId?: string }) => {
      Sentry.logger.info("[SocketHealth] Received room-ping", {
        roomId: data.roomId,
        socketId: socket.id,
