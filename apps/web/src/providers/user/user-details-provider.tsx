...
export function UserDetailsProvider({
  ...
  const fetchUserDetailsFn = useCallback(async () => {
    ...
    try {
      const details =
        await fetchFromActionRoute<UsersAPI.UserDetails.GetMe.Response>(
          "/api/users/details",
        );
      store.setUserDetails(details);
    } catch (error) {
      store.setError(
        resolveActionErrorMessage(error, tRoot, "errors.fetchUserDetails"),
      );
    }
  ...
  const updateUserDetailsFn = useCallback(
    async (data: UsersAPI.UserDetails.PatchMe.Body): Promise<UserDetails> => {
      const updated = await fetchFromActionRoute<UserDetails>(
        "/api/users/details",
        {
          method: "PATCH",
  ...
  const value = useMemo<UserDetailsContextValue>(() => {
    return {
      fetchUserDetails: fetchUserDetailsFn,
