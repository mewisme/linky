...
function BlockedUserActionsCell({
  row,
  callbacks,
}: {
  row: { original: BlockedUserWithDetails };
...
export function useBlockedUsersColumns(
  callbacks?: RowCallbacks,
): ColumnDef<BlockedUserWithDetails>[] {
  const t = useTranslations("dataTable");
  return useMemo(
    () => [
      {
        ...
        cell: ({ row }) => {
          return (
            <span className="text-muted-foreground text-sm">
              {new Date(row.original.blocked_at).toLocaleString("en-US", {
                month: "short",
                day: "numeric",
                year: "numeric",
              })}
      ...
      },
      {
        id: "actions",
        cell: ({ row }) => (
          <BlockedUserActionsCell row={row} callbacks={callbacks} />
        ),
        enableHiding: false,
        enableSorting: false,
      },
