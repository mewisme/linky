...
function CallHistoryActionsCell({
  row,
}: {
  row: { original: CallHistoryRecord };
}) {
...
export function useCallHistoryColumns(
  _callbacks?: RowCallbacks,
): ColumnDef<CallHistoryRecord>[] {
  const t = useTranslations("dataTable");
  return useMemo(
    () => [
      {
          ...
          return (
            <div className="flex items-center gap-3">
              <Avatar className="h-10 w-10">
                <AvatarImage
                  src={row.original.other_user?.avatar_url || ""}
                  alt={row.original.other_user?.name || ""}
                />
                <AvatarFallback>
                  {row.original.other_user?.name?.charAt(0)}
                </AvatarFallback>
              </Avatar>
              <div className="flex flex-col">
                <span className="font-medium">
                  {row.original.other_user?.name}
                </span>
                <span className="text-xs text-muted-foreground flex items-center gap-1">
                  <CountryFlag
                    countryCode={row.original.other_user?.country || ""}
                  />{" "}
                  {row.original.other_user?.country}
                </span>
              </div>
            </div>
          );
        ...
        cell: ({ row }) => {
          return (
            <span className="text-muted-foreground text-sm">
              {new Date(row.original.started_at).toLocaleString("en-US", {
                month: "short",
                day: "numeric",
                hour: "2-digit",
                minute: "2-digit",
