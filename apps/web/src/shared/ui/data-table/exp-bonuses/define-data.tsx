...
function ExpBonusActionsCell({
  row,
  callbacks,
}: {
  row: { original: ExpBonus };
...
function ExpBonusTypeBadge({
  ...
  if (normalized === "level") {
    return (
      <Badge
        variant="outline"
        className="gap-1 border-yellow-500/40 bg-yellow-500/10 text-yellow-800 dark:text-yellow-400"
      >
  ...
  if (normalized === "favorite") {
    return (
      <Badge
        variant="outline"
        className="gap-1 border-pink-500/40 bg-pink-500/10 text-pink-700 dark:text-pink-400"
      >
...
export function useExpBonusColumns(
  callbacks?: RowCallbacks,
): ColumnDef<ExpBonus>[] {
  const t = useTranslations("dataTable");
  return useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const min = formatExpBonusBound(
            config.min,
            t("expBonuses.unboundedMin"),
          );
          const max = formatExpBonusBound(
            config.max,
            t("expBonuses.unboundedMax"),
          );
          return (
