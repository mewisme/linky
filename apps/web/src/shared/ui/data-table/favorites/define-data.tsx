...
function FavoritesActionsCell({
  row,
  callbacks,
}: {
  row: { original: ResourcesAPI.Favorites.FavoriteWithStats };
...
export function useFavoritesColumns(
  callbacks?: RowCallbacks,
): ColumnDef<ResourcesAPI.Favorites.FavoriteWithStats>[] {
  const t = useTranslations("dataTable");
  return useMemo(
    () => [
      {
        ...
        cell: ({ row }) => {
          return (
            <span className="font-medium text-primary">
              {formatDuration(row.original.total_duration || 0)}
            </span>
          );
        },
      },
        ...
        cell: ({ row }) => {
          return (
            <span className="font-medium text-muted-foreground">
              {formatDuration(Math.round(row.original.average_duration || 0))}
            </span>
          );
        },
      },
      ...
      },
      {
        id: "actions",
        cell: ({ row }) => (
          <FavoritesActionsCell row={row} callbacks={callbacks} />
        ),
      },
    ],
    [callbacks, t],
