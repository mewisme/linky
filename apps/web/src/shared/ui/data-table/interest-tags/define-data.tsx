...
function InterestTagActionsCell({
  row,
  callbacks,
}: {
  row: { original: InterestTag };
...
export function useInterestTagColumns(
  callbacks?: RowCallbacks,
): ColumnDef<InterestTag>[] {
  const t = useTranslations("dataTable");
  return useMemo(
    () => [
      {
        ...
        cell: ({ row }) => {
          return (
            <div className="flex items-center gap-3">
              <span className="text-xl p-2 bg-muted rounded-lg">
                {row.original.icon || "🏷️"}
              </span>
        ...
        cell: ({ row }) => {
          return (
            <Badge variant="secondary" className="px-3 py-1 font-medium">
              {row.original.category || t("interestTags.general")}
            </Badge>
          );
        },
      },
      {
        ...
        cell: ({ row }) => {
          return (
            <div className="max-w-[250px] truncate text-muted-foreground italic">
              {row.original.description || t("interestTags.noDescription")}
            </div>
          );
        },
      },
        ...
        cell: ({ row }) => {
          return (
            <Badge variant="outline" className="text-muted-foreground px-1.5">
              ...
              {row.original.is_active
                ? t("interestTags.active")
                : t("interestTags.inactive")}
            </Badge>
          );
        },
      },
