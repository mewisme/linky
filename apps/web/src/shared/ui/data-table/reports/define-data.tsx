...
function ReportsActionsCell({
  row,
}: {
  row: { original: ResourcesAPI.Reports.Report };
}) {
...
): ColumnDef<ResourcesAPI.Reports.Report>[] {
  const t = useTranslations("dataTable");
  return useMemo(
    () => [
      {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return <div className="max-w-[300px] truncate">{reason}</div>;
        },
      },
      {
        accessorKey: "status",
        ...
        cell: ({ row }) => {
          const status = row.getValue(
            "status",
          ) as ResourcesAPI.Reports.ReportStatus;
          return (
            <Pill>
