...
function parseKeywords(
  ...
  return (tRoot as (k: string) => string)(`commandMenu.keywords.${key}`)
    .split(/\s+/)
    .filter(Boolean);
}
...
export function CommandMenu() {
  ...
  const {
    auth: { signOut, isSignedIn },
  } = useUserContext();
  const { user: userStore } = useUserStore();
  const { setCollapsible, setVariant, variant, collapsible } =
    useSidebarStore();
        ...
        ? [
            {
              ...
              actions: [
                ...
                {
                  id: "admin-bonuses",
                  label: t("sidebar.items.adminExpBonuses.label"),
                  icon: IconBolt,
    ...
    <>
      <InputGroup onClick={open}>
        <InputGroupInput
          ...
          placeholder={t("commandMenu.ui.searchPlaceholder")}
          readOnly
        />
        <InputGroupAddon>
          <Search />
      ...
      >
        <Command>
          <CommandInput
            placeholder={t("commandMenu.ui.commandInputPlaceholder")}
          />
          <CommandList>
