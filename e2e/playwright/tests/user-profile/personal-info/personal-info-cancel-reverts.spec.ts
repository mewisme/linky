...
test.describe('Profile — Personal Info', () => {
  ...
  test('Cancel reverts personal info changes', async ({ page }) => {
    ...
    await dateInput.fill('2005-05-05');

    await personalInfo.getByRole('button', { name: /cancel/i }).click();

    await expect(personalInfo.getByRole('button', { name: /save/i })).not.toBeVisible();
  });
});
