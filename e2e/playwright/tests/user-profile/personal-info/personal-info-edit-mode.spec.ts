...
test.describe('Profile — Personal Info', () => {
  ...
  test('Click edit opens inline editing for date of birth and gender', async ({ page }) => {
    ...
    await expect(personalInfo.getByRole('button', { name: /save/i })).toBeVisible();
    await expect(personalInfo.getByRole('button', { name: /cancel/i })).toBeVisible();
  });
});
