...
def test_avatar_fallback_shows_first_letter_when_no_image(driver):
    """Profile — Avatar Management: Avatar fallback shows first letter when no image"""
    """Profile — Avatar Management: Avatar fallback shows first letter when no image"""
    authenticate_user(driver, TEST_USERS['user1'])
    driver.get(settings.base_url + '/user/profile')
    wait_page_loaded(driver)
    fallback = driver.find_elements(By.CSS_SELECTOR, '[data-testid="profile-avatar"], .avatar-fallback')
    assert fallback
