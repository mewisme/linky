...
def test_empty_state_when_no_blocked_users(driver):
    """Blocked Users Page: Empty state when no blocked users"""
    """Blocked Users Page: Empty state when no blocked users"""
    ...
    empty = driver.find_elements(
        By.XPATH,
        "//*[contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'no blocked users')"
        " or contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'empty')]",
    )
    table = driver.find_elements(By.CSS_SELECTOR, 'table, [role="table"]')
    assert empty or table
