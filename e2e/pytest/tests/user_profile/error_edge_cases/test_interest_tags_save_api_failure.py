...
def test_interest_tags_save_api_failure_shows_error_toast(driver):
    """Error & Edge Cases: Interest tags save API failure shows error toast"""
    """Error & Edge Cases: Interest tags save API failure shows error toast"""
    ...
    with block_url_pattern(driver, '**/api/**/user/interest-tags**'):
        save_button_in(interests).click()
        wait_for_text(driver, r"update failed|error", timeout=5)
    assert save_button_in(interests).is_displayed()
