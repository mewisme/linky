...
def test_tag_catalog_fetch_failure_shows_error_state(driver):
    """Error & Edge Cases: Tag catalog fetch failure shows error state"""
    """Error & Edge Cases: Tag catalog fetch failure shows error state"""
    ...
    with block_url_pattern(driver, '**/api/**/interest-tags**'):
        WebDriverWait(driver, 10).until(lambda d: save_button_in(interests).is_displayed())
