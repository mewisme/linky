...
def test_no_tags_found_shows_empty_state(driver):
    """Profile — Interest Tags: No tags found shows empty state"""
    """Profile — Interest Tags: No tags found shows empty state"""
    ...
    search_inputs = driver.find_elements(
        By.XPATH,
        "//input[@type='search' or contains(translate(@placeholder, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'search')]",
    )
    visible = [i for i in search_inputs if i.is_displayed()]
    if visible:
        visible[0].clear()
        visible[0].send_keys("xyzzy123nonexistent")
        wait_for_text(driver, r"no tags found", timeout=5)
