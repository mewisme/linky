...
def test_vietnamese_blocked_users_page_loads_translated(driver):
    """Internationalization: Vietnamese blocked users page loads translated"""
    """Internationalization: Vietnamese blocked users page loads translated"""
    authenticate_user(driver, TEST_USERS['user1'])
    driver.get(settings.base_url + '/vi/connections/blocked-users')
