...
def test_vietnamese_locale_loads_translated_profile_page(driver):
    """Internationalization: Vietnamese locale loads translated profile page"""
    """Internationalization: Vietnamese locale loads translated profile page"""
    authenticate_user(driver, TEST_USERS['user1'])
    driver.get(settings.base_url + '/vi/user/profile')
    wait_page_loaded(driver)
    assert bio_section(driver).is_displayed()
    assert personal_info_section(driver).is_displayed()
    assert interests_section(driver).is_displayed()
