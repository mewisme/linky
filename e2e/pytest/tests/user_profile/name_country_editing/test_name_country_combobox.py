...
def test_country_combobox_search_and_select(driver):
    """Profile — Name & Country Editing: Country combobox search and select"""
    """Profile — Name & Country Editing: Country combobox search and select"""
    ...
    comboboxes = driver.find_elements(
        By.XPATH,
        "//*[@role='combobox' and contains(translate(@aria-label, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'country')]",
    )
    if not comboboxes:
        comboboxes = driver.find_elements(By.CSS_SELECTOR, '[role="combobox"]')
    combobox = comboboxes[0]
    combobox.click()
    combobox.send_keys("Japan")
    option = driver.find_element(
        By.XPATH,
        "//*[@role='option'][contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'japan')]",
    )
    option.click()
    value = combobox.get_attribute("value") or combobox.text
    assert re.search(r"japan", value, re.I)
