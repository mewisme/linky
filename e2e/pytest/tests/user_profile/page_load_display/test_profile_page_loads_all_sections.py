...
def test_profile_page_loads_with_all_sections_visible(driver):
    """Profile Page — Page Load & Display: Profile page loads with all sections visible"""
    """Profile Page — Page Load & Display: Profile page loads with all sections visible"""
    ...
    WebDriverWait(driver, 10).until(
        EC.visibility_of_element_located(
            (By.XPATH, "//img[contains(translate(@alt, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'avatar')]")
        )
    )
    wait_for_text(driver, r"Bio")
    wait_for_text(driver, r"Personal Info")
    wait_for_text(driver, r"Interests")
