...
def test_cancel_reverts_personal_info_changes(driver):
    """Profile — Personal Info: Cancel reverts personal info changes"""
    """Profile — Personal Info: Cancel reverts personal info changes"""
    ...
    date_input.clear()
    date_input.send_keys("2005-05-05")
    cancel_button_in(pinfo).click()
    WebDriverWait(driver, 10).until(
        EC.invisibility_of_element_located(
            (By.XPATH, '//section[@aria-label="Personal Info"]//button[contains(translate(normalize-space(.), "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "save")]')
        )
    )
