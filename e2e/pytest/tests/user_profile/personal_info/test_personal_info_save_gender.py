...
def test_select_a_gender_and_save(driver):
    """Profile — Personal Info: Select a gender and save"""
    """Profile — Personal Info: Select a gender and save"""
    ...
    selects = pinfo.find_elements(By.CSS_SELECTOR, "select")
    if selects:
        Select(selects[0]).select_by_value("female")
    else:
        combobox = pinfo.find_element(By.CSS_SELECTOR, '[role="combobox"]')
        combobox.click()
        option = driver.find_element(
            By.XPATH,
            "//*[@role='option'][contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'female')]",
        )
        option.click()
