...
test.describe('Profile — Bio Section', () => {
  ...
  test('Save bio updates display', async ({ page }) => {
    ...
    await textarea.fill('My updated bio text for testing');
    await bioSection.getByRole('button', { name: /save/i }).click();

    await expect(page.getByText(/bio updated/i)).toBeVisible({ timeout: 10000 });
    await expect(bioSection.getByText(/My updated bio text for testing/)).toBeVisible();
  });
});
