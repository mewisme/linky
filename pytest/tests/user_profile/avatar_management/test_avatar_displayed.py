...
def test_avatar_is_displayed(driver):
    """Profile — Avatar Management: Avatar is displayed"""
    """Profile — Avatar Management: Avatar is displayed"""
    authenticate_user(driver, TEST_USERS['user1'])
    driver.get(settings.base_url + '/user/profile')
    wait_page_loaded(driver)
    avatar = driver.find_elements(By.CSS_SELECTOR, '#avatar-upload, [data-testid="profile-avatar"]')
    imgs = driver.find_elements(By.XPATH, "//img[contains(translate(@alt, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'avatar')]")
    assert avatar or imgs
