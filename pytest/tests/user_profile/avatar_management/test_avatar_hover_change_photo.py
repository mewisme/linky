...
def test_hover_shows_change_photo_overlay(driver):
    """Profile — Avatar Management: Hover shows "Change photo" overlay"""
    """Profile — Avatar Management: Hover shows"""
    authenticate_user(driver, TEST_USERS['user1'])
    driver.get(settings.base_url + '/user/profile')
    wait_page_loaded(driver)
    containers = driver.find_elements(By.CSS_SELECTOR, '[data-testid="profile-avatar"], .profile-avatar')
    assert containers
    el = containers[0]
    hover_section(driver, el)
    wait_for_text(driver, r"change photo", timeout=5)
