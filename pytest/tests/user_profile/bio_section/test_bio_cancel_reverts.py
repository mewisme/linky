...
def test_cancel_edit_reverts_bio(driver):
    """Profile — Bio Section: Cancel edit reverts bio"""
    """Profile — Bio Section: Cancel edit reverts bio"""
    ...
    textarea = sec.find_element(By.TAG_NAME, "textarea")
    textarea.clear()
    textarea.send_keys("Changed text that should be reverted")
    cancel_button_in(sec).click()
    WebDriverWait(driver, 10).until(EC.invisibility_of_element_located((By.CSS_SELECTOR, 'section[aria-label="Bio"] textarea')))
