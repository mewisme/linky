...
def test_blocked_users_list_displays_entries(driver):
    """Blocked Users Page: Blocked users list displays entries"""
    """Blocked Users Page: Blocked users list displays entries"""
    authenticate_user(driver, TEST_USERS['user1'])
    driver.get(settings.base_url + '/connections/blocked-users')
    wait_page_loaded(driver)
    table = driver.find_element(By.CSS_SELECTOR, 'table, [role="table"]')
