...
def test_unblock_a_user_removes_them_from_the_list(driver):
    """Blocked Users Page: Unblock a user removes them from the list"""
    """Blocked Users Page: Unblock a user removes them from the list"""
    ...
    if visible:
        visible[0].click()
        wait_for_text(driver, r"user unblocked", timeout=10)
