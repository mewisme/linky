...
def test_escape_key_cancels_editing(driver):
    """Profile — Name & Country Editing: Escape key cancels editing"""
    """Profile — Name & Country Editing: Escape key cancels editing"""
    ...
    first_name.send_keys("EscapeTest")
    first_name.send_keys(Keys.ESCAPE)
    WebDriverWait(driver, 10).until(
        EC.invisibility_of_element_located(
            (By.XPATH, "//input[contains(translate(@placeholder, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'first name')]")
        )
    )
