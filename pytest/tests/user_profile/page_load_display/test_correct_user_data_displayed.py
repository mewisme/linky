...
def test_correct_user_data_is_displayed(driver):
    """Profile Page — Page Load & Display: Correct user data is displayed"""
    """Profile Page — Page Load & Display: Correct user data is displayed"""
    authenticate_user(driver, TEST_USERS['user1'])
    driver.get(settings.base_url + '/user/profile')
    wait_page_loaded(driver)
    user = TEST_USERS['user1']
    expected_name = " ".join(filter(None, [user.first_name, user.last_name]))
    if expected_name:
        wait_for_text(driver, re.escape(expected_name))
    if user.email:
        wait_for_text(driver, re.escape(user.email))
