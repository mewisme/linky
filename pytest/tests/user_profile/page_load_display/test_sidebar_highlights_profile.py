...
def test_sidebar_highlights_the_profile_item(driver):
    """Profile Page — Page Load & Display: Sidebar highlights the profile item"""
    """Profile Page — Page Load & Display: Sidebar highlights the profile item"""
    authenticate_user(driver, TEST_USERS['user1'])
    driver.get(settings.base_url + '/user/profile')
    wait_page_loaded(driver)
    nav = driver.find_element(By.TAG_NAME, "nav")
    profile_links = nav.find_elements(
        By.XPATH,
        ".//a[contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'profile')]",
    )
    assert profile_links and profile_links[0].is_displayed()
