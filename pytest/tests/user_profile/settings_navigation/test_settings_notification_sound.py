...
def test_notification_sound_toggle_works(driver):
    """Settings Navigation: Notification sound toggle works"""
    """Settings Navigation: Notification sound toggle works"""
    authenticate_user(driver, TEST_USERS['user1'])
    driver.get(settings.base_url + '/settings/notifications')
    wait_page_loaded(driver)
    toggles = driver.find_elements(
        By.XPATH,
        "//*[@role='switch' and contains(translate(@aria-label, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'sound')]"
        " | //input[contains(translate(@aria-label, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'sound')]",
    )
    visible = [t for t in toggles if t.is_displayed()]
    if visible:
        visible[0].click()
        button_by_name(driver, r"save").click()
        wait_for_text(driver, r"settings updated", timeout=10)
